...
void setup(){
  ...
  Serial.println(" ####################################################################");
  Serial.println(" # Mazda 2 Instrument Cluster that allows you to go vroom vroom     #");
...
void loop(){
  ...
  if((millis() - prevTx) >= invlTx){
    prevTx = millis();
    byte txData[8] = {counter, counter, counter, counter, counter, counter, counter, counter};
